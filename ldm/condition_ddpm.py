...
class ConditionDDPM(pl.LightningModule):
    ...
    def shared_step(self, batch):
        # x, cond_dict = self.get_input(batch)
        x, cond_dict = batch
        loss, loss_dict = self(x, **cond_dict)
        return [loss, loss_dict], cond_dict["cond"]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch % 10 == 0 and batch_idx == 1:
            ...
            shape = (self.batch_size, self.channels, self.pad_channel, self.image_size, self.image_size)
            reconstructions, _ = ddim_sampler.sample(50, batch_size=1, shape=shape, conditioning=cond, verbose=False)
    ...
    def test_step(self, batch, batch_idx):
        ...
        shape = (self.batch_size, self.channels, self.pad_channel, self.image_size, self.image_size)
        reconstructions = self.p_sample_loop(cond=cond, shape=shape)
        # reconstructions = self.decode_first_stage(cond_z)
